...
export default function Header() {
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'space-between',position: 'fixed', alignItems: 'center', backgroundColor: 'primary.main', padding: '10px 30px', top: 0, width: '100%' }}>
            <Logo
                sx={{
                    height: '100%',
            ...
            <Drawer anchor="right" open={isDrawerOpen} onClose={toggleDrawer}>
                <Box sx={{ width: 250, height: '100%', padding: '10px', backgroundColor: 'primary.main' }}>
                    <Search />
                    <Box sx={{width: '92%', display: 'flex', justifyContent:'space-between', position: 'absolute', bottom: '10px'}}>
                        <AddArtwork />
                        <LogOutButton />
                    </Box>
                </Box>
